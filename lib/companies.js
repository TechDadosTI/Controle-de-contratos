...
// "label" é o nome curto que aparece na tela (menu, barra lateral, crachá do cabeçalho).
// "razaoSocial" é o nome completo da empresa, usado para preencher a Empresa Contratante ao
// abrir um contrato novo - por isso vem em maiúsculas, como se escreve em contrato.
export const COMPANIES = {
  agrobiotech: { label: 'Agrobiotech', razaoSocial: 'AGROBIOTECH AGRONEGOCIO LTDA' },
  pilar: { label: 'Pilar', razaoSocial: 'PILAR CENTRO DE SERVIÇO COMPARTILHADO LTDA' },
  tarponFranca: { label: 'Tarpon Franca', razaoSocial: 'TARPON AGRO LTDA - FRANCA' },
  tarponAraxa: { label: 'Tarpon Araxá', razaoSocial: 'TARPON AGRO LTDA - ARAXÁ' },
  bgc: { label: 'BGC', razaoSocial: 'BGC EMPREENDIMENTOS E PARTICIPAÇÕES LTDA' },
  agroBgc: { label: 'AgroBGC', razaoSocial: 'AGRO BGC PARTICIPAÇÕES SOCIETÁRIAS S.A.' },
  cempra: { label: 'Cempra', razaoSocial: 'CENTRO EMPRESARIAL PRADO' },
  goldenDrops: { label: 'Golden Drops', razaoSocial: 'GOLDEN DROPS AGROSCIENCE LTDA' },
  biosulfa: { label: 'Biosulfa', razaoSocial: 'BIOSULFA BRASIL LTDA' },
  ...
  agrobics: { label: 'Agrobrics', razaoSocial: 'AGROBRICS AGROBUSINESS LTDA' },
  hidrossoluvel: {
    label: 'Hidrossolúvel',
    razaoSocial: 'HIDROSSOLÚVEL COMERCIO, IMPORTAÇÃO E EXPORTAÇÃO LTDA',
  },
  ...
  // A razaoSocial de cada lista de representantes é copiada da empresa dona no fim do arquivo,
  // junto com o visual.
};
...
Object.entries(COMPANY_REPS).forEach(([empresa, rep]) => {
  COMPANIES[rep].razaoSocial = COMPANIES[empresa].razaoSocial;
...
// Razões sociais das 11 empresas, na ordem do menu. É a lista oferecida no campo "Empresa
// Contratante" para quem precisar trocar por outra empresa do grupo.
export const RAZOES_SOCIAIS = COMPANY_ORDER.map((k) => COMPANIES[k].razaoSocial);
